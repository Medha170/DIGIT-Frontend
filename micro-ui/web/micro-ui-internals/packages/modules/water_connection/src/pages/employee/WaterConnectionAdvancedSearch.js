...
import { newConfig } from "../../configs/WaterConnectionAdvancedSearchConfig";
...
const AdvancedCreate = () => {
  ...
  const onSubmit = (data) => {
    ...
    const onError = (resp) => {
      history.push(`/${window.contextPath}/employee/water_connection/response?isSuccess=${false}`, { message: "TE_CREATION_FAILED" });
    };

    const onSuccess = (resp) => {
      history.push(`/${window.contextPath}/employee/water_connection/response?appNo=${resp.contracts[0].supplementNumber}&isSuccess=${true}`, {
  ...
  return (
    <FormComposerV2
      ...
      text={"WaterConnection Text if required"}
